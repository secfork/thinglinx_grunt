Date.prototype.dateAfter = function(a, b) {
    ...
    var q = a;
    a.$on("$stateChangeStart", function(a, b, c, d, e) { console.log("stateChangeStart", a, b, c, d, e) });
...
}]).controller("show_system_basic", ["$scope", "$sys", "$show", "$state", function(a, b, c, d) { a.$popNav(a.system.name + "", d) }]).controller("show_system_current", ["$scope", "$show", "$interval", "$sys", "$state", "$filter", "$timeout", function(a, b, c, d, e, f, g) {
    function h(c) {
        console.log(k);
        var d;
...
}]).controller("das_basic", ["$scope", "$filter", "$state", "$stateParams", function(a, b, c, d) {}]).controller("das_config", ["$scope", "$state", "$stateParams", "$source", "$modal", "$filter", function(a, b, c, d, e, f) {
    ...
    return { restrict: "AC", template: '<span class="bar" ></span>', link: function(a, d, e) { d.addClass("butterbar hide"), a.$on("$stateChangeStart", function(a) { b.hash("app"), c(), d.removeClass("hide").addClass("active") }), a.$on("$stateChangeSuccess", function(a, b, c, e) { a.targetScope.$watch("$viewContentLoaded", function() { d.addClass("hide").removeClass("active") }) }) } }
...
}]), angular.module("app.sysconfig", [], function() {}).service("$sys", ["$translate", function(a) {
    return {
        manageMode: 1,
        itemsPerPage: 20,
        ...
        permissionDesc: { READ_DATA: "读取系统变量的数据", WRITE_DATA: "将数据写入系统变量中", ALARM_VIEW: "查看系统产生的报警", ACK_ALARM: "确认系统产生的报警", SYSTEM_MANAGE: "创建、删除系统，修改系统基本信息", TICKET_MANAGE: "创建及删除系统的ticket", REGION_USER_MANAGE: "为区域创建用户，以及管理用户权限", SYN_CONFIG: "将系统工程同步到DAServer或网关", SYSTEM_CONTROL: "系统数据下置及召唤，系统的激活及失效，切换系统配置项，保存网络参数等", REGION_MANAGE: "创建、删除区域，修改区域基本信息", MODEL_MANAGE: "创建、修改以及删除系统模型和设备模型", USER_MANAGE: "创建、删除用户，修改用户基本信息以及管理用户权限", ROLE_MANAGE: "创建、修改以及删除角色" },
        accountP: ["REGION_MANAGE", "MODEL_MANAGE", "USER_MANAGE", "ROLE_MANAGE"],
        regionP: ["READ_DATA", "WRITE_DATA", "ALARM_VIEW", "ACK_ALARM", "SYSTEM_MANAGE", "TICKET_MANAGE", "REGION_USER_MANAGE", "SYN_CONFIG", "SYSTEM_CONTROL"],
                ...
                AreaCC: function(a, b, c) {
                    var d = { 6: !0, 8: !0, 9: !0 },
                        ...
                        f = a.params.area;
                         ...
                         // ( b._dataType || b.$parent._dataType )
                         // ( b._accessType || b.$parent._accessType )

                    b  =  c ? b : b.$parent ;

                    d[f] ? b._dataType = this.type : ( 
                            b._dataType = [{ k: "位读写", v: 0 }], 
                            !c && (a.params.type = 0)
                        ),
                    e[f] ? ( b._accessType = [{ k: "Read", v: 0 }], 
                             !c && (a.params.access = 0) 
                            ) :  b._accessType = this.access
